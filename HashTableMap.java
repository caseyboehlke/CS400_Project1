...
/*
 * This class represents a Hash Table that stores key-value pairs
 */
public class HashTableMap<KeyType, ValueType> implements MapADT<KeyType, ValueType> {
   ...
   */
  private void grow(LinkedList<KeyValue>[] array2) {
      ...
      for (int i = 0; i < oldCapacity;i++) {
        ...
        for (int j = 0; j < oldArray[i].size();j++) {
          ...
          if (oldArray[i].get(j) != null) {
            ...
            KeyType oldKey = (KeyType) ((KeyValue) oldArray[i].get(j)).getKey(); // copies the key
                                                                                 // of the KeyValue
                                                                                 // object

            
            KeyValue keyValueNode = ((KeyValue) oldArray[i].get(j)); // copies the KeyValue object
            ...
            if (newArray[index] == null) {
              newArray[index] = new LinkedList<KeyValue>();
              newArray[index].add(keyValueNode);


            }


            newArray[index].add(keyValueNode); //otherwise add it to the list at that index
  ...
  public boolean put(KeyType key, ValueType value) {
    ...
    if (this.array[index] == null) {
      this.array[index]= new LinkedList<KeyValue>(); //creates a linked list at the index
      this.array[index].add(new KeyValue(key, value)); //adds the KeyValue object to the list
      this.size++;
    ...
    this.array[index].add(new KeyValue(key, value));
    this.size++;
  ...
  public ValueType get(KeyType key) throws NoSuchElementException {
    ...
    for (int i = 0; i < array[index].size();i++) {
      ...
      if (array[index].get(i).getKey().equals(key)) {
        ...
        return (ValueType) array[index].get(i).getValue(); 
    ...
    if (containsKey(key)) {
      ...
      for (int i = 0; i < array[index].size(); i++) {
        ...
        if (array[index].get(i).getKey().equals(key)) {
          ...
          returnVar = (ValueType) array[index].get(i).getValue(); // sets returnVar to the KeyValue
